:+1::tada: 首先，感谢您抽出宝贵时间为 **华师手册** 做出贡献！ :tada::+1:

这篇文章将主要叙述的是参与 **华师手册** 编写的写作过程。请您在撰稿或者修正页面之前，仔细阅读以下内容，以帮助您完成更高质量的内容。
...
我们珍惜每位贡献者的热情和付出，也理解大家的专业水平不尽相同。让我们携手合作，共同呵护这个知识的乐园，用准确、专业的内容去帮助更多读者。

## 参与方式

如果您不太了解 markdown 语法，或者没有 Github 账号，可以通过以下方式贡献。
...
1.  打开 [SCNU-SW/SCNU-SW-Wiki](https://github.com/SCNU-SW/SCNU-SW-Wiki) 仓库，点击键盘上的<kbd>.</kbd>按钮（或者将 URL 中的 `github.com` 更改为 `github.dev`）[^ref1]，进入 GitHub 的网页版 VS Code 编辑器；
...
4.  提交后会在网页上方的中央弹出一个提示框，在第一次的提示框内填写标题，第二次的提示框内填写此提交要提交到的仓库内分支名称，之后右下角会弹出一个提示框，内容类似于 `Created Pull Request #1 for SCNU-SW/SCNU-SW-Wiki.`，点击蓝字链接即可查看该 Pull Request。
...
6.  提交后会在网页上方的中央弹出一个提示框，在第一次的提示框内填写标题，第二次的提示框内填写此提交要提交到的仓库内分支名称，之后右下角会弹出一个提示框，内容类似于 `Created Pull Request #1 for SCNU-SW/SCNU-SW-Wiki.`，点击蓝字链接即可查看该 Pull Request。

#### 向 Pull Request 追加更改

1.  打开 [SCNU-SW-Wiki 的 Pull Request 列表](https://github.com/SCNU-SW/SCNU-SW-Wiki/pulls)，找到您提交的 Pull Request 并点击。
2.  Pull Request 页面的标题下方将会有一段例如 `<您的ID> wants to merge x commits into SCNU-SW-Wiki:main from <您的ID>:patch-1` 的文字，点击 `<您的ID>:patch-1` 部分。
...
通常情况下，如果您需要添加一个新页面，或者修改已有页面在目录中的链接，您就需要对 [`mkdocs.yml`](https://github.com/SCNU-SW/SCNU-SW-Wiki/blob/master/mkdocs.yml) 文件作出改动。
...
3.  请您仔细阅读 [贡献指南](https://github.com/SCNU-SW/SCNU-SW-Wiki/blob/master/.github/CONTRIBUTING.md) 和 [社区公约](https://github.com/SCNU-SW/SCNU-SW-Wiki/blob/master/CODE_OF_CONDUCT.md)，并在同意后勾选 PR 模板中的框，表示您同意了以上指南和公约。
